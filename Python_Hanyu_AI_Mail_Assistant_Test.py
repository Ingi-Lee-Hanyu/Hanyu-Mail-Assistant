...
if not st.session_state.logged_in:
    ...
    # --- 로그인 화면 ---
 그렇군요! 그 이유는 지금 코드에서 text-align: center;를 빼버리셨기 때문입니다.

HTML에서 <h1> 태그는 기본값이 왼쪽 정렬입니다. 그래서 중앙으로 가라는 명령 없이 margin-right만 주면, 왼쪽 벽에 딱 붙은 상태에서 오른쪽 공간만 비워두게 됩니다.

가장 깔끔하게 "중앙에서 살짝만 오른쪽으로" 옮기는 정답 코드를 드릴게요. 이 코드로 덮어쓰기 해보세요.

🛠️ 시각적 중앙을 잡는 최종 코드
이번에는 text-align: center로 중앙을 먼저 잡고, padding-left를 이용해 미세하게 오른쪽으로 밀어내겠습니다.
...
st.markdown("""
    <h1 style='
        text-align: center; 
        width: 100%;
        padding-left: 40px;  /* 이 숫자를 늘리면 오른쪽으로 조금씩 더 이동합니다 */
        color: #1A3482; 
        font-size: 3rem; 
        font-weight: 700; 
        margin-bottom: 20px;
